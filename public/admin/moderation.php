...
$totalQ =
    $PDO->prepare("SELECT count( CASE WHEN `is_positive`=0 THEN 1 ELSE NULL END ) as `neg`, count( CASE WHEN `is_positive`=1 THEN 1 ELSE NULL END ) as `pos`FROM `review` WHERE `is_published` = 0  $sql_filter");
$totalQ->execute();
...
$rev1 =
    $PDO->prepare("SELECT * FROM `review` WHERE `is_published` = 0 and `is_positive` = 1 $sql_filter ORDER BY `review_date` DESC LIMIT $show OFFSET $list");
$rev1->execute();

//получаем положительные отзывы
$rev2 =
    $PDO->prepare("SELECT * FROM `review` WHERE `is_published` = 0 and `is_positive` = 0 $sql_filter ORDER BY `review_date` DESC LIMIT $show OFFSET $list");
$rev2->execute();
?>
<!DOCTYPE html>
		...
		<div class="block mod_title" style="padding:15px; width:99.2%">
			...
			<select class="filter_moderation" id="filter_moderation">
                ...
                while ($row = $queryCompany->fetch())
                {
                    ...
                    $countRewiev =
                        $PDO->prepare("SELECT count(*) FROM `review` WHERE `company_id` = :id and `is_published` = 0");
					...
					<div class="block">
						<div class="header-mod">
							<div class="header-mod__user"><?= $row['reviewer_name'] ?> об
								<span><?= infoCompany($row['company_id'],
                                        $PDO)->name ?></span></div>
                      ...
                      <span><?php
                          ...
                          ?></span>
							...
							<button class="submit submit--red" data-id="<?= $row['id'] ?>"
							        data-key="<?= substr(md5($row['id']), 0, 8); ?>" data-type="1">Отключить
							</button>
						</div>
					</div>
                <?php } ?>
			...
			<div class="revFlex__item">
                <?php while ($row = $rev2->fetch()) { ?>
					<div class="block">
						<div class="header-mod">
							<div class="header-mod__user"><?= $row['reviewer_name'] ?> об
								<span><?= infoCompany($row['company_id'], $PDO)->name ?></span></div>
							<div class="header-mod__reveiw--<? echo $row['is_positive'] == 1 ? 'green' :
                                'red'; ?>"><? echo
                                $row['is_positive'] == 1 ? 'Положительный' : 'Отрицательный'; ?></div>
						</div>
